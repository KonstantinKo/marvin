...
def check_network_connection(server="www.google.com"):
    """
    Checks if marvin can connect a network server.
...
def get_pip_requirements(fname=os.path.join(marvin.support.path.LIB_PATH,
                                            'requirements.txt')):
    """
...
def run():
    ...
    logger.info("Starting marvin diagnostic at %s" % time.strftime("%c"))
    ...
    for fname in [os.path.join(marvin.support.path.APP_PATH, os.pardir, "phonetisaurus",
                               "g014b2b.fst")]:
